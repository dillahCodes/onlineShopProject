...
const HomePageTopUpTabs = () => {
      ...
      children: (
        <div className={`flex ${isMobile && "flex-wrap"} items-center w-full gap-x-5`}>
          <div className="flex flex-col w-full gap-y-2">
            <label htmlFor="shopeepay" className="font-medium capitalize truncate font-space-grotesk">
              nomor telepon
            </label>
            <InputComponent size="large" id="shopeepay" type="number" />
          </div>
          <div className="flex flex-col w-full gap-y-2 " id="priceSelect">
            <label htmlFor="shopeepayNominal" className="font-medium capitalize font-space-grotesk">
              nominal
            </label>
            <Select
  ...
  return (
    <div className={`lg:w-[50%] w-full ${isMobile && "mt-5"} `}>
      ...
      <Tabs
        className={`w-full  ${!isMobile && "mt-5"} p-3 rounded-md shadow-sm border-[2px] border-black`}
        defaultActiveKey="0"
        size={"small"}
        items={tabsItems}
      />
...
export default HomePageTopUpTabs;
