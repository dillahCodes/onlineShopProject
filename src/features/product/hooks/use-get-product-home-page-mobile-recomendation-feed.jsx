...
const useGetProductHomePageMobileRecomendationFeed = (
  categoryFeed,
  fetchLimit,
) => {
  ...
  const categoryFeedNames = homePageNavigationFeed.map((item) =>
  ...
  const getCategoryFeedForYou = useCallback(async () => {
    if (!history_search || !user) {
      ...
      try {
        setIsLoading(true);
        const products =
          await productServices.getAllProductsWithLimit(fetchLimit);
  ...
  const getProductBeliLokal = useCallback(async () => {
    ...
    try {
      ...
      const response = fetchLimit
        ? await productServices.searchProductWithLimit(
            selectedCategory,
            fetchLimit,
          )
  ...
  const getProductSmilarCategory = useCallback(async () => {
    ...
    const categoryByCurrentUserSearch =
      history_search[history_search.length - 1].category;
  ...
  useEffect(() => {
    if (isForYou) getCategoryFeedForYou();
    if (isBeliLokal) getProductBeliLokal();
    if (isMiripYangKamuCek) getProductSmilarCategory();
    if (isWebCams || isMakeUpWajah || isMakananJadi)
      getProductByCategory(categoryFeed);
  }, [
    isForYou,
    isMiripYangKamuCek,
    isBeliLokal,
